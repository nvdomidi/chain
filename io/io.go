...
func WriteVerticesToOBJ(v []geom.Vertex, filepath string) error {
...
func ReadObj(filePath string) (geom.Mesh, error) {
	...
	for scanner.Scan() {
		...
		case "v":
			...
			if len(words) != 4 {
				return geom.Mesh{}, fmt.Errorf("Vertices must be 3D")
			}
